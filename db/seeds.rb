...
def display_statistics
  ...
  puts "="*60
  puts "📊 作成されたデータ:"
  puts "  科目: #{Subject.count}件"
  puts "  単元: #{Unit.count}件"
  puts "  問題: #{Question.count}件"
...
def display_question_type_stats
  ...
  Question.question_types.each do |type, value|
    ...
    puts "  - #{label} (#{type}, 値:#{value}): #{count}問"
  end
end
...
def display_difficulty_stats
  ...
  Question.difficulties.each do |diff, value|
    ...
    puts "  - #{label} (#{diff}, 値:#{value}): #{count}問"
  end
end
...
def display_unit_stats
  ...
  Unit.all.each do |unit|
    ...
    Unit::DIFFICULTY_KEYS.each do |diff|
      label = Question::DIFFICULTY_LABELS[diff.to_s]
      puts "    - #{label}: #{counts[diff]}問"
    end
    puts "    - 合計: #{counts[:total]}問"
  end
end
...
puts "  ✓ 高1 基礎単語（易しい）: #{WORDS_EASY.size}問"
...
puts "  ✓ 高2 応用単語（普通5問 + 難しい10問）: 15問"

create_questions(unit_h3_exam, WORDS_ENTRANCE_EXAM, nil)
puts "  ✓ 高3 大学受験 頻出単語: #{WORDS_ENTRANCE_EXAM.size}問"

puts "✅ 問題: #{Question.count}件作成"
