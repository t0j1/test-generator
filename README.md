...
- ✅ 3種類の印刷モード
  ...
  - ✅ **解答のみ**（講師用）
- ✅ 自動改ページ機能
  - 問題数15問以上: 10問目で改ページ（裏面印刷）
  - 解答数30問以上: 20問目で改ページ（裏面印刷）
- ✅ キオスクモード対応
...
```
1枚目（表）: 問題用紙 1〜10問
1枚目（裏）: 問題用紙 11〜15問 ※問題数15問以上の場合
2枚目（表）: 解答用紙 1〜20問
2枚目（裏）: 解答用紙 21〜30問 ※解答数30問以上の場合
```
...
<% if @test_sheet.question_count >= 15 && index == 10 %>
  <div class="page-break"></div>
<% end %>

# 解答用紙の改ページ（show.html.erb）
<% if @test_sheet.question_count >= 30 && index == 20 %>
