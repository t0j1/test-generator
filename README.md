...
### タブレットUI（キオスクモード最適化）
- ✅ **3ステップのシンプルフォーム**（待機画面 → 科目選択 → 単元+設定 → 印刷）
- ✅ **Soft Card × Minimal デザイン**（Frosted Glass効果、控えめなアニメーション）
- ✅ **プログレスバー表示**（Step 1/3, Step 2/3, Step 3/3）
- ✅ **30秒無操作で自動リセット**（待機画面に戻る）
...
### キオスクモードの動作フロー（新3ステップUI）

```
待機画面: 「テストを作成する」をタップ
   ↓
Step 1: 科目選択（1タップ）【Progress: 1/3】
   ↓
Step 2: 単元選択（1タップ） + 設定確認 → 「テストを作成」（1タップ）【Progress: 2/3】
   ↓
Step 3: 印刷画面表示 → 「すべて印刷」（1タップ）【Progress: 3/3】
   ↓
印刷開始
   ↓
「新しいテストを作成」で待機画面に戻る
   ↓
（30秒無操作の場合は自動的に待機画面に戻る）
```

**合計: 3〜4タップ、約45〜60秒で印刷完了！**

#### 新UIの特徴
- **Soft Card デザイン**: Frosted Glass効果で現代的なデザイン
- **プログレスバー**: 現在のステップが一目でわかる
- **カルーセル式単元選択**: 横スクロールで直感的に選択
- **自動リセット機能**: 30秒無操作で待機画面に戻る（次の人が使いやすい）
- **控えめなアニメーション**: タップ時のフィードバックが分かりやすい
